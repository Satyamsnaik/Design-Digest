...
import { DigestConfig, ExperienceLevel, DateRange, Category } from '../types.ts';
import { CATEGORIES_AND_TOPICS } from '../constants.ts';
import { Clock, Signal, Layers, Target } from 'lucide-react';
...
const DigestConfigurator: React.FC<DigestConfiguratorProps> = ({ config, setConfig, onGenerate, isLoading }) => {
  ...
  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setConfig(prev => ({
      ...prev,
      category,
      topics: [CATEGORIES_AND_TOPICS[category][0]] // default to first topic
    }));
  };
  ...
  const toggleTopic = (topic: string) => {
    setConfig(prev => {
      if (prev.topics.includes(topic)) {
        const remaining = prev.topics.filter(t => t !== topic);
        if (remaining.length === 0) {
           return { ...prev, topics: [CATEGORIES_AND_TOPICS[prev.category][0]] };
        }
  ...
  const levels: { id: ExperienceLevel; title: string; subtitle: string }[] = [
    { id: 'Beginner', title: 'Beginner', subtitle: 'Foundations & Concepts' },
    { id: 'Intermediate', title: 'Intermediate', subtitle: 'Execution & Strategy' },
    { id: 'Advanced', title: 'Advanced', subtitle: 'Systems & Leadership' },
  ];

  const currentTopics = CATEGORIES_AND_TOPICS[config.category] || [];
    ...
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-[0_8px_30px_rgba(0,0,0,0.02)] border border-stone-100 max-w-xl mx-auto mb-12 relative overflow-hidden">
      ...
      <div className="space-y-8">
        ...
        <div>
           ...
           <div className="relative">
             <select 
               ...
               className="w-full appearance-none bg-stone-100/70 border border-stone-200 text-charcoal py-3 px-4 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-stone-400/20 active:bg-stone-100 transition-all cursor-pointer"
             >
               {(Object.keys(CATEGORIES_AND_TOPICS) as Category[]).map(cat => (
        ...
        <div>
          ...
          <div className="bg-stone-100/70 p-1.5 rounded-2xl flex flex-wrap md:flex-nowrap relative">
            {levels.map((lvl) => {
