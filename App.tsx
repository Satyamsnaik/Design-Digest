...
import { History, Bookmark, Home, AlertTriangle, Link, Tag, ChevronRight, Filter, RefreshCw, Search, X, Settings, Terminal } from 'lucide-react';
...
// Main App Component
function AppContent() {
  const [view, setView] = useState<'dashboard' | 'history' | 'result' | 'saved' | 'spec'>('dashboard');
  ...
  const handleAnalyzeUrl = async (url: string) => {
    ...
    try {
      ...
      const historyItem: DigestHistoryItem = {
        id: generateId(),
        timestamp: Date.now(),
        config: { category: config.category, level: config.level, topics: [], dateRange: 'Any Time' }, 
        articles: [result],
        type: 'url'
      };
  ...
  const getFilteredHistory = () => {
    ...
    return history.filter(item => {
        // Search in config level/topics/category (safe checks for legacy data)
        if (item.config?.category?.toLowerCase().includes(lower)) return true;
    ...
    <nav className="fixed top-0 inset-x-0 z-50 px-4 py-4 md:py-6 pointer-events-none">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="pointer-events-auto min-w-[40px] min-h-[44px] flex items-center">
             {(view === 'saved' || view === 'history' || view === 'spec') ? (
               <h1 className="font-display text-2xl md:text-3xl font-bold text-charcoal tracking-tight capitalize animate-in fade-in slide-in-from-left-4 duration-500">
                 {view === 'saved' ? 'Saved Articles' : view === 'history' ? 'History' : 'Project Specification'}
               </h1>
             ) : null}
        </div>
          ...
          <div className="space-y-8 md:space-y-12 animate-in fade-in duration-700">
            <section className="text-center space-y-4 md:space-y-6">
              ...
              <p className="text-stone-500 text-lg md:text-xl font-sans font-light max-w-lg mx-auto leading-relaxed px-4">
                Curated intelligence for professionals, designers, and strategists.
              </p>
            </section>
